...
const discoveryStoreBib = (bib) => {
  ...
  const isBib = makeStatementGroupIdentifier(['nypl:Item', 'nypl:Collection'])
  ...
  const statementExtractors = [
    (new BibsUpdater()).extractStatements(new SierraBib(bib))
  ]
    .concat(bib.items.map((item) => (new ItemsUpdater()).extractStatements(new SierraItem(item))))
  ...
  return Promise.all(statementExtractors)
    .then((statements) => {
      ...
      const bib = discoveryStoreModelFromStatements(groups.filter(isBib).shift(), DiscoveryStoreBib)
  ...
  const researchOrCirc = bibs.reduce((researchOrCirc, bib) => {
    ...
    if (locationCodes.includes('none') || locationCodes.includes('os')) isResearch = true
      ...
      const branchLocations = locationCodes
        ...
        .filter((location) => Array.isArray(location.collectionTypes) && location.collectionTypes[0] === 'Branch')
      // If bib has any locations with just "Branch" collection type, bib is not research:
      if (branchLocations.length > 0) isResearch = false
    }
