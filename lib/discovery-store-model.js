...
function nestBlankNodes (statements) {
  ...
  const rootStatements = grouped
    ...
    .map((statement) => {
      ...
      // above, link it:
      if (statement.object_id && statement.object_id.includes('#')) {
        statement.blanknode = blankNodeGroups
          .find((group) => group[0].subject_id === statement.object_id)
        // Wrap it in a Base
        statement.blanknode = new DiscoveryStoreBase(statement.blanknode)
      }
      return statement
    })
...
const discoveryStoreBib = (bib) => {
  ...
  return (new BibsUpdater()).extractStatements(new SierraBib(bib))
    .then((bibStatements) => {
      ...
      logger.debug('Got electronic items: ', groupStatementsBySubjectId.length ? JSON.stringify(groupedElectronicItemStatements, null, 2) : 'None')
 ...
 * those items that are Research
 */
const filterOutNonResearchItems = async (items) => {
  ...
  return Promise.all(
    ...
    logger.info(`From original ${originalCount} item(s), removed ${originalCount - items.length} circulating, resulting in ${items.length} research items`)
    return items
  })
}
 ...
 */
const filterOutNonResearchBibs = async (bibs) => {
  ...
  return Promise.all(
    bibs.map((bib) => {
      ...
      if (bib.locations && Array.isArray(bib.locations) && bib.locations.length > 0) {
        ...
        if (researchLocations.length === 0) return Promise.resolve(null)
      }

      // Otherwise, it *may* be Research after inspecting items, so resolve it:
      return Promise.resolve(bib)
    })
  ).then((bibs) => {
    // Filter out null (non-research) items
    bibs = bibs.filter((bib) => bib)
    if (originalCount) logger.info(`From original ${originalCount} bib(s), removed ${originalCount - bibs.length} circulating, resulting in ${bibs.length} research items`)
    return bibs
  })
}
...
const buildDiscoveryStoreBibs = (bibs) => {
  return filterOutNonResearchBibs(bibs)
    .then((bibs) => {
      return Promise.all(
        bibs.map((bib) => {
