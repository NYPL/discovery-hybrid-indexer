...
const filterOutAndDeleteNonResearchBibs = async (bibs) => {
  ...
  const researchOrCirc = bibs.reduce((researchOrCirc, bib) => {
    ...
    if (bib.locations && Array.isArray(bib.locations) && bib.locations.length > 0) {
      ...
      if (bib.locations[0] && ['none', 'os'].includes(bib.locations[0].code)) isResearch = false
      ...
      isResearch = researchLocations.length > 0
    }
  ...
  if (originalCount) {
    ...
    await Promise.all(
      ...
      logger.error(`Error deleting all/some of ${researchOrCirc.circ.length} bib(s): ${researchOrCirc.circ.join(',')}`, e)
    })
  }

  // console.log('Returning bibs...', researchOrCirc.research)
