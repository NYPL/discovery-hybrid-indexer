...
const attachRecapCustomerCodes = require('./attach-recap-to-items.js')
 ...
 */
const filterOutNonResearchBibs = (bibs) => {
  const originalCount = bibs.length
  return Promise.all(
    bibs.map((bib) => {
      // Partner bibs are Research
      if (bib.nyplSource !== 'sierra-nypl') return Promise.resolve(bib)
    ...
    if (originalCount) logger.info(`From original ${originalCount} bib(s), removed ${originalCount - bibs.length} circulating, resulting in ${bibs.length} research item(s)`)
    return bibs
  })
}
