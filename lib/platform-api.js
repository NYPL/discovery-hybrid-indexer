...
const holdingsForBib = (bib, offset = 0) => {
  // Holdings service does not have holdings for partner bibs
  if (bib.nyplSource !== 'sierra-nypl') return Promise.resolve([])
    ...
    return client.get(`holdings?bib_id=${bib.id}`)
      .then((resp) => {
        ...
        if (!Array.isArray(holdings)) {
          logger.error(`Error fetching holdings for bib_id: ${bib.id}`, holdings)
          return []
        }
        logger.debug('Got ' + (holdings ? holdings.length : 'no') + ' holdings')
        return holdings || []
      })
  })
}
