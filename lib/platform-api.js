...
/**
 *  Given an array of bibs, fetches holdings for all of them into memory so
 *  that subsequent calls on holdingsForBib can use the already-fetched
 *  data rather than querying the HoldingsService. Memory is reset on each call
 */
let HOLDINGS_CACHE = {}
const prefetchHoldingsForBibs = (bibs) => {
  ...
  HOLDINGS_CACHE = bibs.reduce((h, bib) => {
    ...
    if (bib.nyplSource !== 'sierra-nypl') {
      logger.debug('prefetchHoldingsForBibs resolving early for partner bib: ', bib.nyplSource)
      h[bibIdentifier] = Promise.resolve([])
    } else {
      logger.debug('prefetchHoldingsForBibs resolving active all request: ', bib.nyplSource)
      h[bibIdentifier] = fetchAllHoldings
        .then((holdingsGrouped) => {
          return holdingsGrouped
        })
        .then((holdingsGrouped) => holdingsGrouped[bibIdentifier] || [])
    }
    return h
  }, {})
}
